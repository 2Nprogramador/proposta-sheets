...
def salvar_dados_gsheets(df_novos_dados):
    ...
    try:
        ...
        # Converte as datas para string antes de enviar
...
def gerar_dados_proximo_dia(df_atual):
    ...
    for _ in range(qtd_transacoes):
        ...
        linha = {
            ...
            "Gender": gender, "Product line": product_line, "Unit price": unit_price_str,
            "Quantity": int(quantity), "Total": total_str, "Time": time_str,
...
# --- FUNÇÃO DE ALERTAS (MOVIMENTO PARA ESCOPO GLOBAL) ---
...
# =================================================================
# BLOCO DE API: INTERCEPTA O n8n E DEVOLVE DATA + VARIAÇÃO + ALERTAS
# =================================================================
...
if "request_type" in st.query_params:
    ...
    # ---------------------------------------------------------
    # ROTA 2: BUSCAR ALERTAS
    # ---------------------------------------------------------
    elif request_type == "get_alerts" and target_date:
        relatorio_api = relatorio_por_dia_com_variacoes(pd.to_datetime(target_date), df)
        
        if not relatorio_api:
            st.json({"alertas_positivos": [], "alertas_negativos": [], "total_alertas": 0, "status": "sem_dados"})
            st.stop()
...
# --- ALERTAS NA INTERFACE (Usando a mesma função da API) ---
dict_alertas_ui = calcular_alertas_dia(relatorio)
alertas_positivos = dict_alertas_ui["alertas_positivos"]
alertas_negativos = dict_alertas_ui["alertas_negativos"]
total_alertas = dict_alertas_ui["total_alertas"]
...
# Funções de Estilo e Plotagem para o Streamlit
...
def style_generic(df_input, col_name, format_str):
    ...
    if is_first_day:
        ...
        format_dict[col_var] = format_str.replace("{:", "{:+")
    return df_input.style.format(format_dict, na_rep="-")
...
def plot_total_and_variation(df_total, df_var, id_col, title):
    ...
    fig = px.bar(df_plot, x=id_col, y='Valor', color='Métrica', barmode='group', title=f"{title}", template='plotly_white', color_discrete_map=color_map, labels={'Métrica': 'Variável'})
...
def plot_generic(df_main, df_var, id_col, val_col, title, color_main='rgb(50, 168, 82)', color_var='rgb(50, 168, 140)'):
    ...
    if 'index' in df_concat.columns:
        df_concat.rename(columns={'index': id_col}, inplace=True)
    df_plot = df_concat.melt(id_vars=id_col, value_vars=[val_col, col_var_name], var_name='Métrica', value_name='Valor').dropna(subset=['Valor'])
    color_map = {val_col: color_main, col_var_name: color_var}
    fig = px.bar(df_plot, x=id_col, y='Valor', color='Métrica', barmode='group', title=title, template='plotly_white', color_discrete_map=color_map)
    fig.update_layout(height=400, title_x=0.5, margin=dict(l=20, r=20, t=40, b=20))
    fig.add_hline(y=0, line_dash="dash", line_color="gray")
    return fig
