...
def gerar_dados_proximo_dia(df_atual):
    ...
    for _ in range(qtd_transacoes):
        ...
        linha = {
            ...
            "Quantity": int(quantity), "Total": total, "Time": time_str,
            "Payment": payment, "Rating": rating, "Data": data_registro
        }
...
if "Pix" in relatorio['total_por_payment'].index:
    ...
    variacao_pix = relatorio['variacao_payment'].loc["Pix", "Total"] if "Pix" in relatorio['variacao_payment'].index else 0
...
with st.expander("Alertas Importantes", expanded=True if total_alertas > 0 else False, icon="🚨"):
    for alerta in alertas_positivos: st.success(alerta)
    for alerta in alertas_negativos: st.error(alerta)
    if not total_alertas: st.info("Nenhum alerta foi gerado para o dia selecionado.")

# --- PLOTS E INSIGHTS ---
st.subheader(f"Relatório Detalhado de Vendas para o dia {dia_selecionado}")

# 1. CÁLCULO PRÉVIO DOS INSIGHTS (FILTRO SEGURO)
...
def plot_total_and_variation(df_total, df_var, id_col, title):
    ...
    if 'index' in df_concat.columns: df_concat.rename(columns={'index': id_col}, inplace=True)
    df_plot = df_concat.melt(id_vars=id_col, value_vars=['Total', 'Var. Total', 'Quantity', 'Var. Quantity'], var_name='Métrica', value_name='Valor').dropna(subset=['Valor'])
    color_map = {'Total': 'rgb(76, 120, 168)', 'Quantity': 'rgb(30, 60, 100)', 'Var. Total': 'rgb(228, 87, 86)', 'Var. Quantity': 'rgb(190, 40, 40)'}
    fig = px.bar(df_plot, x=id_col, y='Valor', color='Métrica', barmode='group', title=f"{title} - Total, Quantidade e Variação", template='plotly_white', color_discrete_map=color_map, labels={'Métrica': 'Variável'})
...
with col1:
    ...
    with st.expander("Gráfico"):
        ...
        st.plotly_chart(px.bar(df_plot, x='City', y='count', color='Customer type', facet_col='Gender', barmode='group', title='Distribuição de Clientes', labels={'count': 'Número de Clientes'}), use_container_width=True)

    # === INSIGHT 1: Golden Hours ===
    if insights:
        st.markdown("---")
        ...
        with st.expander("Ver Gráfico"):
            fig_hora = px.area(insights['vendas_por_hora'], x='Hora_Int', y='Total', markers=True, title="Volume de Vendas ao longo do dia", color_discrete_sequence=['#FF4B4B'])
            fig_hora.update_xaxes(tickmode='linear', dtick=1)
            st.plotly_chart(fig_hora, use_container_width=True)

    # === INSIGHT 3: Qualidade vs Faturamento ===
    if insights:
        st.markdown("---")
        ...
        with st.expander("Ver Matriz de Qualidade"):
            fig_qualidade = px.scatter(
                ...
                size='Faturamento', color='Product line', title=f"Produtos ({dia_selecionado})", hover_name='Product line'
            )
            media_geral = df_dia_raw['Rating'].mean()
            fig_qualidade.add_hline(y=media_geral, line_dash="dot", annotation_text="Média Geral")
...
with col2:
    ...
    with st.expander("Gráfico"):
        ...
        st.plotly_chart(px.bar(df_plot, x="City", y="value", color="Customer type", barmode="group", title="Distribuição de Clientes"), use_container_width=True)
    ...
    with st.expander("Gráfico"):
        ...
        fig = px.bar(df_plot, x='City', y='count', color='Gender', facet_col='Payment', barmode='group', template='plotly_dark')
    ...
    if insights:
        ...
        with st.expander("Ver Gráfico"):
            fig_ticket = px.bar(insights['ticket_medio_tipo'], x='Customer type', y='Ticket_Medio', color='Customer type', text_auto='.2f', title="Quem gasta mais por visita?")
